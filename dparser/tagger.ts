    ...
    private match(strs: string[]) {
        ...
        while(strs.length > 0) {
            ...
            if(cps && cps.length>0) {
                // for phrases
                for(let cp of cps) {
                    if(cp.partOfSpeech === POSTags.verb && cp.elements[cp.elements.length-1].pos === POSTags.particle) {
                        cp.elements[cp.elements.length-1].setTag(Tagset.PVRP)
                        cp.elements[cp.elements.length-1].setWordForm(buf[1])
    
                        if(cp.elements[cp.elements.length-1].wordForm != cp.elements[cp.elements.length-1].lexeme.word.literal) {
                            const ls = tonal_lemmatization_analyzer.doLexicalAnalysis(buf[0])
                            cp.elements[0].lexeme = tonal_inflextion_analyzer.doAnalysis(ls[0].lemmata[0].literal, new TonalCombiningForms(), new TonalInflexion())[0]
                        } else {
                            cp.elements[0].lexeme = tonal_inflextion_analyzer.doAnalysis(buf[0], new TonalCombiningForms(), new TonalInflexion())[0] 
                        }
