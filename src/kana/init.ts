...
function getKanasFollowingSmallTsu(key: string) {
  const kanaSequences: string[] = ["", "", ""];
  let ks = kogakimoji.get("tsu");
  if (ks) {
...
export function getKanaBlocks(morphemes: KanaStandaloneMorpheme[]): string[] {
  ...
  for (const m of morphemes) {
    ...
    if (ks != undefined && ks[0] != undefined) {
      ...
      if (
        m.sounds[0].toString() === m.sounds[1].toString() &&
        geminatedConsonantsKana.includes(m.sounds[0].toString()) == true
      ) {
        // e.g. ddo, ggu
        const got = getKanasFollowingSmallTsu(
        ...
        m.sounds[1].toString() === KanaLetterTags.ts &&
        geminatedConsonantsKana.includes(m.sounds[0].toString()) == true
      ) {
        // e.g. ttsu
        const got = getKanasFollowingSmallTsu(
