...
describe('Epenthesis testing', () => {
  ...
  test('check the consonant', () => {
    expect(ta1.letterSequences[1][0].toString()).toEqual(TonalLetterTags.l);
  });

  const ta2 = cli.processTonal('jiokxgay');
  test('check the consonant', () => {
    expect(ta2.letterSequences[1][0].toString()).toEqual(TonalLetterTags.g);
  });

  const ta3 = cli.processTonal('kapfbay');
  test('check the consonant', () => {
    expect(ta3.letterSequences[1][0].toString()).toEqual(TonalLetterTags.b);
  });
...
describe('Voiced final consonant testing', () => {
  ...
  test('check if it handles sandhi final consonants', () => {
    expect(ta.letterSequences[0].map(it => it.toString()).join('')).toEqual(
      'teg'
    );
  });
});
...
describe('-ik ending testing', () => {
  ...
  test('check if it handles sandhi rime, tik of tikw, tikw of tekk', () => {
    expect(ta1.letterSequences[0].map(it => it.toString()).join('')).toEqual(
      'tik'
    );
  });
  ...
  test('check if it handles sandhi final consonants, chik of chikf, chikf ot chit', () => {
    expect(ta2.letterSequences[0].map(it => it.toString()).join('')).toEqual(
      'chik'
    );
  });
});
